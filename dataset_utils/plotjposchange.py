...
def pick_pitcher(intent_path, name):
    ...
    if not name in pchr_list:
        raise Exception("This name has no record.")
...
def main():
    ...
    pitcher_name = raw_input("Give me a name (HJ, LSC, LYX, MY, XH, ZL): ")
    pitcher_path = pick_pitcher(intent_path, pitcher_name)
    trial_index = input("Give me a trial indicator (from 0 to 9): ")
